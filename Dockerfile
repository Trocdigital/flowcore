...
RUN apt-get -y update && apt-get -y update && apt-get -y install sudo netcat-traditional libmagic-dev libgeos-dev pkg-config  git  libaio1 libaio-dev default-libmysqlclient-dev locales locales-all make automake gcc g++ subversion libpq-dev postgresql-client-common postgresql-client unixodbc unixodbc-dev libsqliteodbc \
